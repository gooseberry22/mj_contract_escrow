...
export function Payments({ onBack, onNavigate }: PaymentsProps) {
  ...
  // Calculate escrow balance from shared data
  const escrowBalance = `$${getTotalEscrowBalance().toLocaleString()}`;
  
  // All transactions from shared data (same source as surrogate)
  const paymentHistory = completedPayments.map(p => ({
    date: p.date,
